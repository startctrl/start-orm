...
use start\db\exception\DataNotFoundException;
use start\db\exception\DbException as Exception;
use start\db\exception\ModelNotFoundException;
...
abstract class BaseQuery
{
    ...
    public function paginate($listRows = null, $simple = false): Paginator
    {
        ...
        if (is_array($listRows)) {
            ...
            $listRows = intval($config['per_page']);
        } else {
            $config   = $defaultConfig;
            $listRows = intval($listRows ?: $config['per_page']);
        }
        ...
        if (!isset($total) && !$simple) {
            ...
            if ($total > 0) {
                ...
                if (!empty($this->model)) {
                    $results = new \start\model\Collection([]);
                } else {
                    $results = new \start\Collection([]);
                }
            }
        } elseif ($simple) {
    ...
    public function paginateX($listRows = null, string $key = null, string $sort = null): Paginator
    {
        ...
        $listRows = is_int($listRows) ? $listRows : (int) $config['per_page'];
        $page     = isset($config['page']) ? (int) $config['page'] : Paginator::getCurrentPage($config['var_page']);
        $page     = $page < 1 ? 1 : $page;
