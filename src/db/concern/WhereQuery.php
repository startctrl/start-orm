...
namespace start\db\concern;

use Closure;
use start\db\BaseQuery;
use start\db\Raw;
use start\helper\Str;
...
trait WhereQuery
{
     ...
     * @return $this
     */
    protected function parseArrayWhereItems(array $items, string $logic)
    {
        $where = [];
        $hasWhereOr = [];   // 关联OR查询
        $hasWhereAnd = [];  // 关联AND查询
        ...
        foreach ($items as $key => $value) {
            if(is_string($key) && stripos($key, '.') !== false && $alias === ''){
                list($model, $key) = explode('.', $key);
                $alias = Str::studly($table);
                $this->alias($alias);
                $this->hasWhere($model, []);
            }
        }
        ...
        foreach ($items as $key => $val) {
            ...
            if (is_int($key)) {
                $where[] = $val;
                continue;
            } else if (!in_array($key, $tableFields) && stripos($key, '|') === false && stripos($key, '@') === false && stripos($key, '.') === false) {
                continue;
            }
            ...
            if ($val instanceof Raw) {
                $where[] = [$key, 'exp', $val];
            } else if (stripos($key, '|') !== false && stripos($key, '.') !== false) {
                // 关联OR查询
                ...
                foreach ($orFields as $orField) {
                    if (stripos($orField, '.') !== false) {
                        ...
                        $orCondition[$model][] = $this->parseWhereItem($logic, $field, '=', $val);
                    } else {
                        !isset($orCondition[$table]) ? $orCondition[$table] = [] : '';
                        $orCondition[$table][]  = $this->parseWhereItem($logic, ($alias ? $alias.'.' : '').$orField, '=', $val);
                    }
                }
                ...
                $hasWhereAnd[$model][] = $this->parseWhereItem($logic, $field, '=', $val);
                continue;
            } else if (stripos($key, '|') !== false) {
                // OR查询
                $keys = explode('|', $key);
                $this->where(function ($query) use ($alias, $keys, $val) {
                    foreach ($keys as $k) {
                        $query->whereOr(($alias ? $alias.'.' : '').$k, '=', $val);
                    }
                });
            } else {
                // $logic查询
                $where[] =  $this->parseWhereItem($logic, $key, '=', $val);
            }
        }

        // 设置主表别名
        if ($alias && !empty($where)) {
            foreach ($where as &$item) {
                if ($item[0] && stripos($item[0], '.') === false) {
                    $item[0] = ($alias ? $alias.'.' : '') . $item[0];
                }
            }
        }
        ...
        if (!empty($hasWhereOr)) {
            foreach ($hasWhereOr as $relation) {
                $models = array_keys($relation);
                foreach ($models as $model) {
                    if ($model != $table) {
                        $this->hasWhere($model, []);
                    }
                }
                $this->where(function ($query) use ($relation, $table) {
                    foreach ($relation as $model => $condition) {
                        $query->whereOr(function ($query1) use ($table, $model, $condition) {
                            if ($model === $table) {
                                $query1->whereOr($condition);
                            } else {
                                $query1->hasWhere($model, function ($query2) use ($condition) {
                                    $query2->setOption('where', []);
                                    $query2->whereOr($condition);
                                });
                            }
                        });
                    }
    ...
    {
        if (is_array($condition) && !in_array(strtoupper($op), ['BETWEEN', 'NOT BETWEEN', 'BETWEEN TIME', 'NOT BETWEEN TIME'], true)) {
            $op = 'in';
        }
        ...
        if ($op === 'like' || $op === 'not like') {
            $condition = stripos($condition, '%') === false ? '%' . $condition . '%' : $condition;
        }
        if (is_array($op)) {
            // 同一字段多条件查询
